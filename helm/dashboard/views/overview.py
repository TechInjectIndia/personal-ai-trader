"""Helm dashboard — Overview page (intraday bot, paper trading).

Rendered as the default page by the ``st.navigation`` router in ``app.py``.
Keeps its own ``st.set_page_config`` — under st.navigation the router issues no
Streamlit command before ``.run()``, so this is still the first command at run.
"""
